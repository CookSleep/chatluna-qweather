...
var Config = import_koishi.Schema.intersect([
  ...
  import_koishi.Schema.union([
    import_koishi.Schema.object({
      authType: import_koishi.Schema.const("apiKey").required(),
      apiKey: import_koishi.Schema.string().role("secret").default("").description("和风天气 API KEY")
    }).description("API KEY 配置"),
    import_koishi.Schema.object({
      authType: import_koishi.Schema.const("jwt").required(),
      jwtKeyId: import_koishi.Schema.string().default("").description("JWT 凭据 ID，即 Header 中的 `kid`，在凭据详情页获取"),
      jwtProjectId: import_koishi.Schema.string().default("").description("JWT 项目 ID，即 Payload 中的 `sub`，在项目详情页获取"),
      jwtPrivateKey: import_koishi.Schema.string().role("textarea").role("secret").default("").description("Ed25519 私钥 PEM，插件会用它自动生成 Bearer JWT"),
